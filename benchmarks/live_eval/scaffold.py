#!/usr/bin/env python3
"""Deterministic scaffold for the live A/B benchmark playground (asha_eval).

Generates the asha_eval package:
  core.py      - 600+ line module, 3 near-identical handlers sharing
                 byte-identical retry boilerplate (_RETRY_DELAY = 2.5 x3),
                 plus calculate_metrics() and utility functions.
  service.py   - downstream consumer, 5 calculate_metrics call sites.
  test_suite.py- strict pytest suite (behavior, type hints, invariants).

Deterministic: same bytes every run. Stdlib only.
"""
...
HANDLER = '''\
    def {name}(self, event: Event, ctx: Context) -> Result:
        """Handle {channel} events with retry + persistence."""
        log.info("{name} start")
...
# module-level utility functions (real, deterministic, padding to 600+)
UTILS: list[str] = [
...
def build() -> Path:
    """(Re)generate the package; returns the package directory."""
    PKG.mkdir(parents=True, exist_ok=True)

    utilities = "\n\n\n".join(textwrap.dedent(util) for util in UTILS)

    handlers = []
    for letter, banner in (("a", "channel-a"), ("b", "channel-b"), ("c", "channel-c")):
        body = HANDLER.format(name=f"handle_event_{letter}", channel=banner)
    ...
    core_src = (
        ...
        + utilities
        + "\n\n\n"
        + textwrap.dedent(CLASS_HEAD)
        + "\n\n"
        + "\n".join(handlers)
        + "\n"
    )
    (PKG / "core.py").write_text(core_src, encoding="utf-8")
    (PKG / "service.py").write_text(textwrap.dedent(SERVICE), encoding="utf-8")
    (PKG / "test_suite.py").write_text(textwrap.dedent(TESTS), encoding="utf-8")
    (PKG / "__init__.py").write_text(textwrap.dedent(INIT), encoding="utf-8")

    line_count = len(core_src.splitlines())
    assert line_count >= 600, f"core.py too short: {line_count} lines"
    assert core_src.count("_RETRY_DELAY = 2.5") == 3, "handlers not ambiguous"
    return PKG
...
if __name__ == "__main__":
    ...
    print(f"generated {package} "
          f"(core.py: {len((package / 'core.py').read_text(encoding='utf-8').splitlines())} lines, "
          f"3 identical _RETRY_DELAY sites)")