...
def test_human_labels_deterministic_no_claims() -> None:
    for state in ALL_STATES:
        ...
        for banned in ('امن است', 'مشکلی ندارد', 'مستقل است',
                       'همه تست‌ها سالم'):
...
def test_sealed_only_when_projector_sealed() -> None:
    ...
    assert sealed.count('class="step done"') >= 5   # all five steps done
    assert 'class="step current"' not in sealed
    ...
    assert unsealed.count('class="step done"') == 2
    assert unsealed.count('class="step current"') == 1
    assert 'class="step failed"' not in unsealed
    assert 'class="step interrupted"' not in unsealed
...
def test_no_service_fabrication() -> None:
    ...
    for banned in ('RUNNING', 'PASSED', 'Service', 'node_is_running',
                   'سرویس'):
